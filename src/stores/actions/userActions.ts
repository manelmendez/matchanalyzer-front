...
export const userActions = {
  ...
  signIn(credentials: any) {
    ...
    return axios.post('users/signin', {}, { auth: credentials }).then((response) => {
      let authUser = {
  ...
  signUp(data: any) {
    ...
    return axios.post('users', data.body, { auth: data.credentials }).then((response) => {
      let authUser = {
        token: response.data.token,
        isLogged: true,
        ...response.data.user
  ...
  async editUser(body: any) {
    try {
      ...
      const response = await axios.put('users/' + body.id, body)
      userStore.user = { ...response.data.user }
    } catch (error) {
      console.error('Error al hacer la petición edit user')
    }
  },
