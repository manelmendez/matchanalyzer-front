<template>
  <div class="grid gap-8">
    ...
    <div
      class="grid md:grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-8 mt-4"
      v-if="round"
    >
      <div>
        <TopScorers
    ...
    <div>
      <div
        class="grid grid-cols-6 max-xl:grid-cols-4 max-lg:grid-cols-3 max-md:grid-cols-2 gap-2"
      >
        <Card v-for="team in competition?.teams" class="teamCard h-full">
          <template #content>
            <i
              v-if="team.managerId"
              class="material-icons absolute text-yellow-300"
            >
              star
            </i>
...
export default {
  ...
  setup() {
    ...
    const topScorers = computed(() =>
      competitionStore.topScorers(router.currentRoute.value.params.roundId)
    );
    const mostTrashed = computed(() =>
      competitionStore.mostTrashed(router.currentRoute.value.params.roundId)
    );
    const topDifference = computed(() =>
      competitionStore.topDifference(router.currentRoute.value.params.roundId)
    );
